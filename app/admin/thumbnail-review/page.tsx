      ...
      <div style={s.imgWrap}>
        ...
        {state.status === 'generating' && (
          <div style={s.imgPlaceholder}>
            ...
            <div style={{ fontSize: 11, color: '#93c5fd', marginTop: 4 }}>~30–60 seconds</div>
          </div>
        )}
  ...
  useEffect(() => {
    ...
    fetch(`/api/blog/dashboard-data?secret=${secret}`)
      ...
      .then((data) => {
        ...
        const missing = (data.posts as Post[]).filter((p) => !p.hasCoverImage)
        setPosts(missing)
        const initial: Record<string, CardState> = {}
        missing.forEach((p) => { initial[p._id] = { status: 'idle' } })
        setStates(initial)
      })
      ...
      <div style={s.container}>
        ...
        {!loading && posts.length === 0 && (
          <div style={s.allDone}>
            ...
            <div style={{ fontSize: 13, color: '#888884' }}>
              Check back after publishing new posts.
            </div>
          </div>
        )}
        ...
        {!loading && posts.length > 0 && (
          <div style={s.footerNote}>
            Images are generated by Google Gemini using curated Hampton Roads community photos as the background.
            DALL-E 3 is used as a fallback if Gemini fails. Generation takes 30–60 seconds per image.
