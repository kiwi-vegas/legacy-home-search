...
export async function POST(req: NextRequest) {
  try {
    ...
    const { firstName, lastName, email, phone, interest, message, agentSlug } = body

    if (!firstName || !email || !message || !agentSlug) {
      return NextResponse.json({ error: 'Missing required fields' }, { status: 400 })
    }

    const agent = await getTeamMember(agentSlug)
    if (!agent?.email) {
    ...
    await resend.emails.send({
      ...
      to: agent.email,
      replyTo: email,
      subject: `New inquiry from ${firstName} ${lastName} — ${agentName} · Legacy Home Search`,
      html: `
<!DOCTYPE html>
<html>
...
<body style="margin:0;padding:0;background:#f8f7f4;font-family:Inter,-apple-system,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f8f7f4;padding:32px 16px;">
      ...
      <table width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;background:#ffffff;border-radius:12px;border:1px solid #e0ddd8;">
        ...
        <tr>
          <td style="padding:28px 32px 20px;border-bottom:1px solid #e0ddd8;">
            <div style="font-size:11px;font-weight:700;letter-spacing:0.14em;text-transform:uppercase;color:#2563eb;margin-bottom:8px;">${agentName} · Legacy Home Search — New Contact Inquiry</div>
            <div style="font-size:22px;font-weight:700;color:#1a1a1a;">${firstName} ${lastName}</div>
            <div style="font-size:14px;color:#888884;margin-top:4px;">Submitted via Legacy Home Search agent page</div>
          </td>
        </tr>
        ...
        <tr>
          <td style="padding:8px 32px 28px;">
            <a href="mailto:${email}?subject=Re: Your inquiry to ${agentName} · Legacy Home Team" style="display:block;text-align:center;background:#2563eb;color:#ffffff;font-weight:700;font-size:15px;padding:14px 32px;border-radius:8px;text-decoration:none;">
              Reply to ${firstName} →
            </a>
          </td>
        </tr>
        ...
        <tr>
          <td style="padding:20px 32px;border-top:1px solid #e0ddd8;">
            <div style="font-size:11px;color:#aaa9a4;text-align:center;">
              Legacy Home Search · ${agentName} · Contact Form Submission
            </div>
          </td>
        </tr>
    ...
    console.error('Contact agent form error:', err)
