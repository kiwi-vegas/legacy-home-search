...
interface Props {
  agentName: string   // first name shown in UI, e.g. "Tanya"
  agentSlug: string   // Sanity slug used to look up email server-side, e.g. "tanya-thompson"
  agentPhone: string  // formatted phone for error fallback, e.g. "(757) 737-1866"
  agentPhoneHref: string // tel: href, e.g. "tel:+17577371866"
}

export default function AgentContactForm({ agentName, agentSlug, agentPhone, agentPhoneHref }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch('/api/contact-agent', {
        ...
        body: JSON.stringify({ ...form, agentSlug }),
      })
    ...
    return (
      <div style={{
        ...
        <p style={{ fontSize: 15, color: 'var(--text-secondary)', marginBottom: 24, lineHeight: 1.7 }}>
          {agentName} received your message and will get back to you within a few hours.
        </p>
        <button
  ...
  return (
    <div style={{
      ...
      <h3 style={{ marginBottom: 24 }}>Send {agentName} a Message</h3>
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div className="form-group" style={{ margin: 0 }}>
          ...
          <textarea
            ...
            rows={4}
            placeholder={`Tell ${agentName} a bit about what you're looking for…`}
        ...
        {status === 'error' && (
          <p style={{ fontSize: 13, color: '#dc2626', margin: 0 }}>
            ...
            <a href={agentPhoneHref} style={{ color: '#dc2626' }}>{agentPhone}</a>.
          </p>
        )}
